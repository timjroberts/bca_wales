...
CREATE TABLE deletion_jobs (id TEXT PRIMARY KEY, subject TEXT UNIQUE, status TEXT NOT NULL CHECK(status IN ('pending','complete','failed')), created_at INTEGER NOT NULL, expires_at INTEGER NOT NULL, cursor TEXT);
CREATE TABLE posts (
...
CREATE TABLE media (id TEXT PRIMARY KEY, post_id TEXT NOT NULL REFERENCES posts(id), owner TEXT, policy_version INTEGER NOT NULL, sensitive INTEGER NOT NULL CHECK(sensitive IN (0,1)), manifest TEXT NOT NULL, created_at INTEGER NOT NULL);
...
CREATE TABLE recovery_outbox (id TEXT PRIMARY KEY, action TEXT NOT NULL, target TEXT NOT NULL, payload TEXT NOT NULL, created_at INTEGER NOT NULL, delivered_at INTEGER);
