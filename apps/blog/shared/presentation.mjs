...
export function articleHtml(article) {
  return `<article class="editorial"><div class="article-intro"><a class="back-link" data-nav href="/blog/"><span aria-hidden="true">←</span> All stories</a><p class="eyebrow">BCA Wales · Blog</p><h1>${e(article.title)}</h1><p class="article-deck">${e(article.excerpt)}</p><div class="article-details"><p class="eyebrow">Written by</p><p class="author-name">${attributionHtml(article.attribution)}</p><p class="byline">${article.publishedAt ? `Published ${dateHtml(article.publishedAt)}<br>` : ''}Updated ${dateHtml(article.updatedAt)}</p></div>${article.commentsEnabled === true ? '<a class="discussion-link" href="#comments">Join the conversation <span aria-hidden="true">↘</span></a>' : ''}</div><div class="article-paper"><div class="article-body">${article.html}</div><div class="article-end"><span aria-hidden="true">✳</span><p>Landscape. Community. Recovery.</p><a data-nav href="/blog/">More from BCA Wales →</a></div></div></article>${article.commentsEnabled === true ? '<section id="comments" aria-label="Comments"><h2>Comments</h2><p>Loading comments…</p></section>' : ''}`;
}
