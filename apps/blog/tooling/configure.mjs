...
const config={ name:`bca-wales-blog-${environment}`,account_id:account,main:'dist/worker.mjs',compatibility_date:'2026-08-01',workers_dev:false,preview_urls:false,assets:{ directory:'dist/static',binding:'ASSETS',run_worker_first:true },triggers:{ crons:['0 * * * *'] },observability:{ enabled:false },vars:{ ORIGIN:origin,AUTH_ENABLED:String(auth),COMMENTS_ENABLED:flag('BCA_BLOG_COMMENTS_ENABLED','false'),RESTRICTED:flag('BCA_BLOG_RESTRICTED','true'),PUBLISH_PAUSED:flag('BCA_BLOG_PUBLISH_PAUSED','true'),BACKUPS_ENABLED:flag('BCA_BLOG_BACKUPS_ENABLED','false'),BACKUP_RETENTION_CONFIRMED:flag('BCA_BLOG_BACKUP_RETENTION_CONFIRMED','false'),...(auth?{ FACEBOOK_APP_ID:value('BCA_BLOG_FACEBOOK_APP_ID'),FACEBOOK_GRAPH_VERSION:value('BCA_BLOG_FACEBOOK_GRAPH_VERSION'),FACEBOOK_USER_LINK:'false',SESSION_ACTIVE_KID:value('BCA_BLOG_SESSION_ACTIVE_KID') }:{}) },d1_databases:[{ binding:'DB',database_name:`bca-wales-blog-${environment}`,database_id:database,migrations_dir:'migrations' }],r2_buckets:[{ binding:'CONTENT',bucket_name:`bca-wales-blog-${environment}-content`,jurisdiction:'eu' },{ binding:'RECOVERY',bucket_name:`bca-wales-blog-${environment}-recovery`,jurisdiction:'eu' }],...(images?{ images:{ binding:'IMAGES' } }:{}) };
