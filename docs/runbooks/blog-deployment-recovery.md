...
Test provider lifecycle delivery in a **separate approved window**, with the login deadline/token removed. Set only `STAGING_LIFECYCLE_TEST_UNTIL` to a Unix timestamp at most one hour ahead on the same restricted staging origin. Keep normal authentication disabled and publication paused; the database recovery switch must be false for the test. This exposes only POST `/auth/deauthorize`, POST `/auth/deletion`, and GET/HEAD `/deletion-status/<43-character receipt>`. Meta's app-secret signature authenticates callbacks; no browser access code, session or Origin header is needed. Invalid signatures, duplicate form parameters and stale signed requests cannot create an erasure job. The status page is private/no-store, contains no identity and loads no application script.

Before triggering removal in Facebook's Apps and Websites settings, verify the owner's staging identity has no content or administrator access that should be preserved. App removal/deletion consent is an owner action. Record `facebook_deauthorize_success` and `facebook_deletion_success` timestamps from D1 to distinguish actual accepted deliveries; these contain no payload or account identifier. Check the pending job and durable recovery journal, then run the operator's `maintenance` command if necessary to finish erasure independently of a cron incident. Manual maintenance is not scheduled-delivery evidence. Do not claim both callback types passed from only one timestamp or from locally signed requests. After testing, restore the database restriction, remove the lifecycle deadline and redeploy. Login remains closed throughout.
...
Body image uploads default sensitive, require approved derivatives and never expose originals. Source sensitivity cannot downgrade the asset floor. Replacement uses a fresh asset ID; reclassification of a sensitive asset requires replacement/review. Changes to warning/sensitivity are included in the exact reveal key. Pixelation can still be recognizable: inspect the safe preview and choose the neutral placeholder when necessary. Do not substitute a CSS blur or public original URL if processing fails.

Authors may choose a separate sharing image after confirming `public-sharing-v1`: it must be safe for unrestricted public sharing, with a meaningful description. The editor previews a metadata-stripped 1200 × 630 PNG crop, generated with the Images binding's `fit: cover` transform. Selection is optional; the association card remains the fallback. No body image is selected automatically. A sharing-only asset exposes only its reviewed crop. Uploads and saved corrections stay private until deliberate publication; published sharing URLs require the current revision, and are revoked on replacement, removal, withdrawal or deletion. Third-party caches and already downloaded copies cannot be recalled.

Sharing-image release checks: in staging, upload through an approved account, inspect the real provider crop, save/reload, publish, check direct HTML and client navigation, then remove/replace and withdraw. Check anonymous GET and HEAD of the crop, rejection of other variants/originals, and fallback metadata. Verify backups include the crop and owner erasure/journal replay remove it. Existing file/dimension limits apply; the sharing image counts toward the 30-image document limit. No additional service or permission is required, but provider processing still needs staging verification before release.

Rollback compatibility: this change adds an optional `sharingImage` source field and media variant, with no database migration. Older editors reject documents containing that field without rewriting them, and older Workers cannot serve the new `/share` variant. Before rolling back to a version without sharing-image support, use the compatible editor to remove custom sharing selections, save and republish affected public posts, and retain the original private documents for recovery. If that is not possible during an incident, restrict publication/service according to this runbook and restore a compatible Worker before reopening; do not rewrite source documents or expose originals as a workaround.
