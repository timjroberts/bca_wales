...
The scheduled handler fails visibly when maintenance fails, erasures remain pending or staging leases are stale; it throws only a generic message. Configure Cloudflare scheduled-invocation failure alerts and the owner notification destination before launch. `/api/admin/health` and the operator `status` command expose only aggregate counts/last-backup time. Observe authentication/callback error rates and missing-artifact 503s without logging URL queries, request bodies, cookies, tokens, provider subjects or private content. Automatic full request logging is disabled in generated configs; use metadata-only metrics. The repository validation workflow is not a substitute for hosted monitoring.
