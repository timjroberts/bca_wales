...
The Worker remains disabled. No paid features, staging access controls or production settings changed. The owner subsequently chose to stop standalone repetitions and run a small end-to-end staging check instead. That check passed; see [staging image integration evidence](blog-staging-images.md). The original resource-limit result does not establish maximum-input performance.
